...
// CS3B - midterm-1 fputstring function
// 04/02/2026
...
.global fputstring // Provide program starting address
  ...
  //  X0, and a file descriptor in X1, will output the string to corresponding file
  //  represented by the file descriptor. It calls helper function
  //  String_length which is in the same source file.

  // INPUT:
  //  X0: Must point to a null terminated string
  //  X1: A valid file descriptor
  //  LR: Must contain the return address (automatic when BL
  //      is used for the call)
  //
  // OUTPUT:
  //  The passed string is printed to the designated file descriptor
  //  No values are returned.
  //
  //  All registers except   X0, X1, X2, X3, X4, X5 and X8 are preserved
  //*****************************************************************************
fputstring:
    ...
    MOV X4, X0              // save X0 (string) in X4
    MOV X5, X1              // save X1 (fd) in X5
    ...
    // put the fd in X0 where SVC wants it
    MOV X0, X5              // set output file descriptor to passed fd
